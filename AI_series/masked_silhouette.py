...
def return_pose_landmarks(frame) -> list:
    ...
    if pose_landmarks_class.pose_landmarks:
        ...
        draw_spec_circle, draw_spec_line)
...
def get_my_hands(frame):
    ...
    if multi_hands_landmarks_class.multi_hand_landmarks:  # Breaks the data set in a simple 21 landmarks for each hand.
        ...
        for single_hand_landmarks in multi_hands_landmarks_class.multi_hand_landmarks:
            my_hand = []
            drawing_object.draw_landmarks(frame, single_hand_landmarks, mediapipe.solutions.hands.HAND_CONNECTIONS, 
            draw_spec_circle, draw_spec_line)
...
draw_spec_circle = drawing_object.DrawingSpec(thickness=0, circle_radius=2, color=(0, 0, 255))
draw_spec_line = drawing_object.DrawingSpec(thickness = 2, color=(0, 0, 255))
...
lower_range = np.array([0, 0, 254])
upper_range = np.array([0, 0, 255])
while True:
    ignore,  frame = cam.read()
    my_hands, multi_handedness = get_my_hands(frame)
    my_pose_landmarks = return_pose_landmarks(frame)
    multi_face_mesh = faces_mesh_landmarks(frame)
    my_mask = cv2.inRange(frame, lower_range, upper_range)
    coloured_mask = cv2.bitwise_and(frame, frame, mask=my_mask)
    cv2.imshow('Masked Person', coloured_mask)
    cv2.moveWindow('Masked Person', 0, 0)
