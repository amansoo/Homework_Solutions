# This program moves the window based on the position of the oject of interest (OOI)
...
while True:
    ...
    my_contours, junk = cv2.findContours(my_masks, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  # It is found on the mask because
                                                                                                # the mask has the required point

    for my_contour in my_contours:
        area = cv2.contourArea(my_contour)
        if area >= 50:
            # cv2.drawContours(frame, [my_contour], 0, (255, 0, 0), 3)
            x, y, w, h = cv2.boundingRect(my_contour)
            cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)

    cv2.imshow('Your Object', object)
    cv2.moveWindow('Your Object', int(x / 9 * 16), int(y / 16 * 9))
